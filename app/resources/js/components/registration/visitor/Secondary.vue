        ...
        <form @submit.prevent="submit" class="mb-3">
            <div class="form-row">
                ...
                >
                    ...
                    >
                    <option value="1000"
                        >For additional devices (phones, tablets, CCTV, routers
                        etc)</option
                    >
                    <option value="1001"
            ...
            <div class="form-row">
                <label class="text-bold" for="deRegReason"
                    >Select Issuing Country</label
                >
                <select
                    class="custom-select text-center"
                    id="country-select"
                ...
                >
                    ...
                    <option
                        ...
                        v-bind:value="country.ABBREVIATION"
                        v-bind:key="country.ABBREVIATION"
                        >{{ country.COUNTRY }}</option
                    >
                </select>
                <div v-if="errors && errors.issuingCountry" class="text-danger">
                    {{ errors.issuingCountry[0] }}
                </div>
            </div>
            ...
            >
                <em class="fa mr-2 fas fa-arrow-right"></em> Register Secondary
            </button>
        </form>
    </div>
</template>
...
export default {
    ...
    mounted() {
        setTimeout(() => {
            axios
                .get("/api/immigration-countries")
                .then(response => {
                    this.countries = response.data;
                })
                .catch(error => {
                    this.message =
                        "An error has occured while fetching immigration countries list !";
                });
        }, 2000);
    },
    ...
    methods: {
        submit() {
            ...
            axios
                .post("/api/visitor/register-secondary", this.fields)
                .then(response => {
                    loader.hide();
        ...
        onCountryChange(e) {
            this.fields.issuingCountry = e.target.value;
        }
    }
};
</script>
